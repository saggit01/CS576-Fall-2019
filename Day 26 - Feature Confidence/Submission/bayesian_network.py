...
class BayesianNetwork(graph.AdjacencyListDirectedGraph, graph.VertexLabeledDirectedGraph):
    ...
    def model_evidence(self, data):
        """Returns the model evidence: log P(D)
        
        Assumes uniform Dirichlet prior (alpha = 1) for each set of parameters.
        Args:
            data: a list of observations (tuples)
        """
        ss = self.sufficient_statistics(data)
        return sum(config_model_evidence(counts)
                   for vertex_ss in ss for counts in vertex_ss.values())      
...
def config_model_evidence(counts):
    """The contribution to the model evidence for the counts
    for one configuration of parents for one vertex"""
    return sum(math.lgamma(count + 1) for count in counts) - math.lgamma(sum(counts) + len(counts))
...
def all_possible_networks(names, values):
    """An iterator over all possible Bayesian Networks for the given random variables
    
    Args:
        names: a list of the names of the random variables
        values: a list of the possible values (each a list) for each random variable.
    """
    for dag in graph.all_dags(len(names)):
        network = BayesianNetwork(names)
        for i in dag.topological_order():
            parent_names = [names[j] for j in dag.parents(i)]
            network.set_cpd(names[i], parent_names, values[i])
        yield network
