...
class DirectedGraph:
    ...
    def topological_order(self):
        ...
        def visit(i):
            if visited[i]: return
            if visiting[i]: raise CyclicError
            visiting[i] = True
    ...
    def is_dag(self):
        """Returns true if this graph is a DAG."""
        try:
...
class CyclicError(Exception):
    def __init__(self):
        super().__init__("Graph contains a cycle, and is thus not a DAG")
...
class VertexLabeledDirectedGraph(DirectedGraph):
    ...
    def vertex_labels(self):
        """Returns a list of the labels of the vertices."""
...
def all_dags(n, GraphType=AdjacencyListDirectedGraph):
    """An iterator over all possible DAGs of n vertices."""
    vertex_pair_options = [(None, (i, j), (j, i)) for i in range(n) for j in range(i + 1, n)]
    for edges in itertools.product(*vertex_pair_options):
        g = GraphType(n)
        for edge in edges:
            if edge is not None:
                g.add_edge(*edge)
